...
dBar:function(JAG, D){
	...
	////////////////////////////////////////////////
	D.$Scene.find('div').off('mouseenter mouseleave').on('mouseenter mouseleave',function(e){
		var $El=$(this),
		...
		if(isItem || isAuxChar){
			...
			switch(e.type){			
				...
				case 'mouseenter':
					...
					if((isExit && JAG.Story.joinWord) || (isAuxChar && JAG.Story.ActionWord==='Use')) return;
					if(data.text!==false) $El.updateBar(JAG, 'enter', itemType, data.text); 
					
					////////////////////////////////////
					// VERB HIGHLIGHTING [NO ITEM EXITS]
					////////////////////////////////////
					if(!JAG.Story.ActionWord && data.highlight_verb && !isExit){
				...
				case 'mouseleave': 
					...
					$('#JAG_Panel').find('li.JAG_Word').removeClass('verb_hovered');
	...
	// VERB EVENTS
	//////////////
	$('#JAG_Verbs').find('li').on('mouseenter mouseleave click', function(e){
		...
		switch(e.type){
			...
			case 'mouseenter': 
				$Verb.addClass('verb_hovered');
				if(JAG.Story.ActionWord!=='Use' && JAG.Story.ActionWord!=='Give') $Verb.updateBar(JAG, 'enter', 'actionWord', text); 
			break;
