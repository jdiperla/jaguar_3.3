...
Action:function(JAG, itemType, callback, inInv){
	...
	// CALL CORRECT ACTION
	switch(JAG.Story.ActionWord.toLowerCase().removeWS()){
...
pick_up:function(JAG, $Item, iD, inInv){
	...
	////////////////////////////
		...
		if(iD.pick_up!=false && iD.pick_up!=='_'){
			...
			if(iD.value > 0){ 
				D.gD.experience=D.gD.experience.pF()+iD.value; 
...
look_at:function(JAG, $Item, iD, inInv){
	...
	if(inInv){
		...
		///////////////////////
		if(iD.inv_look_at!=false && iD.inv_look_at!=='_') $Item.actionTimer(JAG, D, iD.inv_look_at, 'inv_look_at_text');
	...
	///////////////////////
		...
		//////////////////////////
		if(iD.look_at!=false && iD.look_at!=='_') $Item.actionTimer(JAG, D, iD.look_at, 'look_at');
...
talk_to:function(JAG, $Item, iD, inInv){
	...
	/////////////////////		
		...
		/////////////////////////
		if(iD.talk_to!=false && iD.talk_to!=='_') $Item.actionTimer(JAG, D, iD.talk_to, 'talk_to');
...
/***********************************************************************************************/
// HELPER FUNCTION - CHECKS CHARACTER CAN RESPOND - $Item.checkSay(iD.actionText);
/***********************************************************************************************/
checkSay:function(JAG, action, D, say, startQuestion){
	...
	for(var i=0; i<action.length; i++){ 
		...
		if(actionX.indexOf('say') <0 || actionX.indexOf('riddle') <0) var toSpeak=false; 
	};
	
	// TALK
	if(toSpeak) $(this).saySomething(JAG, D.$Char, say, JAG.OBJ.$currentScene, startQuestion);	
},
...
actionTimer:function(JAG, D, action, actionText){
	...
	if(actionText!=='talk_to') D.$Char.switchSprite(JAG, D.cD, actionText);
	...
	JAG.Story.ActionTimer=setInterval(function(){ 
		...
		if(elapsed >= 350){
			...
			if(!D.cD.walking) D.$Char.switchSprite(JAG, D.cD, 'stopping');
			...
			if($Item.Achievements(JAG, action, false, actionText)){
				...
				if(actionText==='pick_up') $Item.remove().addToInv(JAG, $Item.data('item'));
			};
		};
	}, 50);	
},
