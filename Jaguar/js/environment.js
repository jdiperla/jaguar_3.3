...
$.fn.extend({
/***********************************************************************************************/
// WEATHER EFFECTS - $Game.weatherEffects(JAG, D, src);
/***********************************************************************************************/
weatherEffects:function(JAG, D, src){
	/////////////////////////////
	...
	var canvas=JAG.OBJ.$Weather[0],
		...
		particleWidth=D.sD.weather_size.split(',')[0].pF(),
		particleLength=D.sD.weather_size.split(',')[1].pF(),
...
DayNightCycle:function(JAG, D){
	...
	/////////////////////////////////////////////////////////////////////////
	if(!D.$Scene.find('div.JAG_DayNightCycle').length) D.$Scene.prepend('<div class="JAG_DayNightCycle"></div>');
	JAG.OBJ.$DayNight=D.$Scene.find('div.JAG_DayNightCycle');
...
/***********************************************************************************************/
// FOG LAYER - D.$Scene.fogEffects(JAG, D);
/***********************************************************************************************/
fogEffects:function(JAG, D){
	...
	D.$Scene.prepend('<canvas id="JAG_Fog" width="'+D.gD.viewportW+'" height="'+D.gD.viewportH+'"></canvas>');	
	JAG.OBJ.$Fog=$('#JAG_Fog');
	...
	JAG.OBJ.$Fog.css({opacity:D.sD.fog_opacity.pF(), visibility:D.sD.fog ? 'visible' : 'hidden' });
...
/***********************************************************************************************/
// TWINKLING STARS LAYER - D.$Scene.stars(JAG, D);
/***********************************************************************************************/
stars:function(JAG, D){
	...
	// CREATE STAR LAYER
	////////////////////
	if(!D.$Scene.find('div.JAG_Stars').length){
		D.$Scene.prepend('<div class="JAG_Stars"></div>');
		JAG.OBJ.$Stars=D.$Scene.find('div.JAG_Stars');		
		...
		$el.sparkle({
			...
			starX:D.sD.star_range.split(',')[0].pF(),
			starY:D.sD.star_range.split(',')[1].pF(),
			direction:D.sD.star_direction.toLowerCase().removeWS()		
		});
	}else{
		D.$Scene.find('div.JAG_Stars')[0].style.visibility="visible";
		JAG.OBJ.$Stars=D.$Scene.find('div.JAG_Stars');				
	};
}});
